...
class Server {
    ...
    try {
      ...
      routesPath.forEach((component) => {
        try {
          ...
          logger.info(`${component} API route not loaded, cause: ${err}`);
        }
      });
    } catch (err) {
  ...
  _loadMongoConnection() {
    mongoConfig.onReady
      .then(() => {
        try {
          // logger.info(`MONGOCONFIG successfully loaded`);
        } catch (err) {
          logger.error(`LOAD MONGOCONFIG FAILED, CAUSE: \n ${err}`);
        }
      })
      .catch((err) => {
        logger.error(`LOAD MONGO FAILED, CAUSE: \n ${err}`);
      });
    return this;
  }
